...
# Pre-requisite:  You need to register an Auth0 account (free) and create your application (domain and api), then do the following:
...
'''
IMPORTANT: You need to get your token by typing in the following AUTH0 URL call and SUBMIT it: 
   ...
   - Download Postman app, open the app and do the following:
       ...
       - once 'Bearer Token' is selected, on the right pane look for Token and enter the token value:
          ...
          - SUCCESS test: good scenario, enter the token value provided in the browser URL, then hit SEND button next to the request URL in Postman
             ...
             - TIP:  If you get an error that says your token has expired, you need to resubmit the AUTH0 URL and pick up the new token returned,
                     repace the bearer token in Postman with the new value, and hit SEND again
'''
